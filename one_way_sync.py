...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = (argparse.ArgumentParser
              (description="One-way synchronization script between two chosen"
                           "directories; after execution the contents of the"
                           "replica directory are modified to match the source"
                           "directory."))
    parser.add_argument('source_dir', type=is_valid_dir,
                        help='The system path of the desired source directory. '
                             'It serves as a template for the replica '
                             'directory. Cannot be a subdirectory of the '
                             'chosen replica directory. An error may be raised '
                             'if the path is not enclosed in quotes.')
    parser.add_argument('replica_dir', type=is_valid_dir,
                        help='The system path of the desired replica '
                             'directory. It is periodically overwritten '
                             '(according to the frequency) and becomes a '
                             'duplicate of the source directory. Cannot be a '
                             'subdirectory of the source directory. An error '
                             'may be raised if the path is not enclosed '
                             'in quotes.')
    ...
    parser.add_argument("-l", "--log_dir", type=str,
                        ...
                        help='If a log file already exists, please enter its '
                             'directory path in quotes. If it doesn\'t, or you'
                             ' would like to create a log in a different '
                             'directory, please enter the desired path in '
...
def check_log_file(log_dir: str) -> None:
    ...
    if os.path.isdir(log_dir):
        ...
        if os.path.isfile(log_file):
            print(f'\nLog found at "{log_file}".\n')
        else:
            add_to_log("Log created.")
            print(f'\nLog created at "{log_file}".\n')

    else:
        log_file = os.path.abspath(os.getcwd()) + "\\" + "sync_log.txt"
...
def copy_dir(dir_a, dir_b) -> None:
    ...
    for dir_a_entry in dir_a_contents:
        path_in_dir_a = dir_a + "\\" + dir_a_entry
        path_in_new_dir = new_dir + "\\" + dir_a_entry
...
def remove_from_dir(removable_paths: list[str], bloated_dir: str) -> None:
    ...
    for removable_entry in removable_paths:
        ...
        if os.path.isfile(removable_entry_path):
            ...
            # In this case removable_entry_path is an empty directory
            os.rmdir(removable_entry_path)
        else:
            # For non-empy directories, remove_from_dir needs to be applied
            # recursively in order to log the files that are being removed.
...
def sync_directories(s_dir: str, r_dir: str) -> None:
    ...
    for source_entry in source_contents:
        s_entry_path = s_dir + "\\" + source_entry
        r_entry_path = r_dir + "\\" + source_entry
    ...
    # files or folders that aren't in s_dir. Hence, by removing the names of
    # files and folders in both s_dir and r_dir from replica_contents during the
...
if __name__ == '__main__':
    ...
    while not is_valid_replica(args.source_dir, args.replica_dir):
        ...
        # characters from the input that may prevent directory verification.
